...
void align_read_to_path(
    ...
    if (cigar_result.empty()) {
        // Observed to happen when using e.g. `WFAligner::MemoryUltralow` on
        // ONT R9 reads.
        non_match_portion_result=1.0;
        return;
    }
    string length_token;
 ...
 */
void align_reads_vs_paths(TransMap& transmap, const VariantGraph& variant_graph, float min_read_hap_identity, int32_t flank_length, const size_t wfa_length_threshold, WFAlignerGapAffine& aligner_lowmem, WFAlignerGapAffine& aligner_highmem, path output_dir=""){
    ...
    transmap.for_each_read([&](const string& read_name, int64_t id) {
        ...
        const size_t read_length = read_sequence.length();
    ...
    for (const auto& [read_sequence, read_ids]: deduplicated_reads){
        ...
        for (const auto& [path_name, path_sequence]: path_sequences) {
            ...
            align_read_to_path(read_sequence, path_sequence, (read_sequence.length()>wfa_length_threshold||path_sequence.size()>wfa_length_threshold)?aligner_lowmem:aligner_highmem, path_evaluation_window, cigar, non_match_portion);
...
//        cerr << alignment.get_path_string() << ' ' << is_spanning << '\n';
        ...
        if (is_new) {
            ...
            size_t rc_length = rc.length();
            for (size_t i=0; i<rc_length; i++) {
                if (rc[i]=='A') rc[i]='T';
                else if (rc[i]=='C') rc[i]='G';
                else if (rc[i]=='G') rc[i]='C';
                else if (rc[i]=='T') rc[i]='A';
                else if (rc[i]=='a') rc[i]='t';
                else if (rc[i]=='c') rc[i]='g';
                else if (rc[i]=='g') rc[i]='c';
                else if (rc[i]=='t') rc[i]='a';
                else { 
                    // NOP: we leave this character unchanged since we don't
                    // know how to complement it.
                }
            }
            result = path_reassignment.find(rc);
            is_new = (result == path_reassignment.end());
        }
...
void write_all_variants_to_vcf(
        VariantGraph& variant_graph,
        const vector<string>& sample_names,
        const path& output_path,
        const Region& region
    ...
    for (auto& record : variant_graph.vcf_records){
        record.format = "GT";
...
void merge_thread_fn(
    ...
    // We observed that long segments from ONT R9 reads make
    // `WFAligner::MemoryHigh` take too much memory. We use this threshold to
    // heuristically switch to `WFAligner::MemoryUltralow` in case of very long
    // read segments, or paths. The chosen value is arbitrary.
    const size_t WFA_LENGTH_THRESHOLD = hapestry_config.interval_max_length;
    // TODO: test these params??
    WFAlignerGapAffine aligner_lowmem(4,6,2,WFAligner::Alignment,WFAligner::MemoryUltralow);
    WFAlignerGapAffine aligner_highmem(4,6,2,WFAligner::Alignment,WFAligner::MemoryHigh);
    ...
    while (i < regions.size()) {
        ...
        align_reads_vs_paths(transmap, variant_graph, hapestry_config.min_read_hap_identity, flank_length, WFA_LENGTH_THRESHOLD, aligner_lowmem, aligner_highmem, subdir / "pre_optimization_alignments");
...
/**
 * Check if any steps in the hapestry log failed
 * @param log_csv path of log.csv generated by hapestry, containing a column called 'success' with 0/1 values
 * @return the log is failed if any row is not a 1 for the success column
 */
bool is_fail(path log_csv) {
    bool fail = false;

    size_t i = 0;
    size_t column_index;
    string column_name = "success";

    if (not exists(log_csv)) {
        return true;
    }

    for_each_row_in_csv(log_csv, [&](const vector<string>& items) {
        if (i == 0) {
            column_index = index_of_first_occurrence(items, column_name);
            if (column_index > items.size()) {
                throw runtime_error("ERROR: 'success' column not found in log: " + log_csv.string());
            }
        ...
        bool i_success = stoi(items[column_index]);
...
/**
 * Locate and concatenate all the subdirectory VCFs in the hapestry output dir
 * @param out_vcf where to write the concatenated VCF
 * @param vcf path to input VCF which contains the proper header to be used in the output
 * @param output_dir path to hapestry dir that contains all the region subdirectories with sub-vcfs to be concatenated
 * @param regions all regions (flanked) that were run by hapestry
 * @param flank_length
 * @param fail_log_suffix what to name the fail log, i.e. windows_[suffix].bed
 * @param vcf_prefix the filename prefix of the subdirectory VCFs so that they can be found properly TODO: automate this?
 */
void concatenate_output_vcfs(
    ...
    for (size_t i=0; i<regions.size(); i++){
        ...
        path sub_log = output_dir / region.to_unflanked_string('_', flank_length) / ("log.csv");

        // Skip if the file does not exist
        if (is_fail(sub_log)){
            // Log that this region did not complete normally (could be total failure or non-optimal solution)

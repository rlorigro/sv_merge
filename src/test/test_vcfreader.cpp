 ...
 */
void test(const string& test_id, const string& chromosome, const bool& is_diploid, const bool& high_qual_only, const float& min_qual, const bool& pass_only, const uint32_t min_sv_length, const uint32_t n_samples_to_load, const float& min_af, const float& min_nmf, path input_vcf, path truth_vcf, path test_vcf) {
    ...
    VcfReader reader(input_vcf,test_callback,10000,chromosome,is_diploid,high_qual_only,min_qual,pass_only,min_sv_length,n_samples_to_load,min_af,min_nmf);
...
void test_single_sample_vcf(const path& caller_vcf, const bool& filter_by_qual, const string& caller_id, const vector<string> CHROMOSOMES, const vector<float> MIN_QUALS, const vector<int> MIN_SV_LENGTHS, path input_vcf, path truth_vcf, path test_vcf) {
    bool is_diploid, high_qual_only, pass_only;
    ...
    for (auto& chromosome: CHROMOSOMES) {
        ...
        if (filter_by_qual) {
            ...
            for (auto& min_qual: MIN_QUALS) {
                ...
                test(caller_id,chromosome,is_diploid,high_qual_only,min_qual,pass_only,min_sv_length,1,min_af,min_nmf,input_vcf,truth_vcf,test_vcf);
            }
        }
        // Filtering by FILTER
        ...
        run_command(command,truth_vcf);
        test(caller_id,chromosome,is_diploid,high_qual_only,min_qual,pass_only,min_sv_length,0,min_af,min_nmf,input_vcf,truth_vcf,test_vcf);
    }
}
...
void test_joint_vcf_hprc(const path& joint_vcf, const vector<string> CHROMOSOMES, const vector<float> MIN_QUALS, const vector<float> MIN_AFS, const vector<float> MIN_NMFS, const string N_THREADS, const path tmp1_vcf, const path tmp2_vcf, const path input_vcf, const path truth_vcf, const path test_vcf) {
    ...
    for (auto& chromosome: CHROMOSOMES) {
        ...
        for (auto& min_qual: MIN_QUALS) {
            ...
            run_command(command,truth_vcf);
            test("HPRC",chromosome,is_diploid,high_qual_only,min_qual,pass_only,min_sv_length,n_samples_to_load,min_af,min_nmf,input_vcf,truth_vcf,test_vcf);
        }
        ...
        command="cut -f 1-9 "+input_vcf.string()+R"( | grep "^[^#]")";
        run_command(command,truth_vcf);
        test("HPRC",chromosome,is_diploid,high_qual_only,min_qual,pass_only,min_sv_length,0,min_af,min_nmf,input_vcf,truth_vcf,test_vcf);
        // Filtering by AF
...
int main(int argc, char* argv[]) {
    path ROOT_DIR = path(argv[1]);  // Assumed to contain every raw input file used for testing
    string N_THREADS = string(argv[2]);

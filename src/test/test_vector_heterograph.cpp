...
int main(){
    ...
    auto id = g.name_to_id("a");

    cerr << "removing node a" << '\n';
    g.remove_node(id);

    cerr << "TESTING: nodes and edges" << '\n';

    // Try exhaustive iteration of nodes to verify a is not present
    g.for_each_node([&](int64_t id, const HeteroNode& node){
        cerr << node.name << '\n';

        if (node.name == "a"){
            throw runtime_error("FAIL: node a is still present");
        }

        g.for_each_neighbor(id, [&](const HeteroNode& neighbor, int64_t id_other){
            cerr << "neighbor: " << neighbor.name << '\n';

            if (neighbor.name == "a"){
                throw runtime_error("FAIL: neighbor a is still present");
            }
        });
    });
